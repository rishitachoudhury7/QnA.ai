...
export function AddResource({
  ...
  const { createResource } = useAppState();
  ...
  return (
    <div className={compact ? "p-1" : "mx-auto max-w-4xl px-6 py-10 lg:px-10"}>
      ...
      <div className="surface mt-6 p-5">
        ...
        {stage === "loading" && <div className="mt-5 rounded-xl bg-neutral-50 p-4 text-sm">Preparing your learning material... <span className="ml-2 text-neutral-400">✓ Resource identified · ● Preparing transcript and timestamps</span></div>}
        {stage === "done" && <div className="mt-5 border-t border-[var(--line)] pt-5"><div className="flex items-center gap-2 text-sm font-semibold text-[#1f7a5a]"><Check size={17} /> Resource ready to prepare</div><div className="mt-5 grid gap-4 sm:grid-cols-3"><div><div className="text-xs text-neutral-400">Resource</div><div className="mt-1 font-semibold">{resourceType === "youtube" ? "YouTube Video" : selectedFile?.name}</div></div><div><div className="text-xs text-neutral-400">Status</div><div className="mt-1 font-semibold">Background preparation</div></div><div><div className="text-xs text-neutral-400">Learning material</div><div className="mt-1 font-semibold">Timestamp-aware</div></div></div><div className="mt-6 flex flex-wrap gap-2"><button onClick={() => void finish(true)} className="inline-flex items-center gap-2 rounded-xl bg-black px-4 py-2.5 text-sm font-semibold text-white">Start Learning <ArrowRight size={15} /></button><button onClick={() => void finish(false)} className="inline-flex items-center gap-2 rounded-xl border border-[var(--line)] bg-white px-4 py-2.5 text-sm font-semibold"><Plus size={15} /> Add to Learning Path</button></div></div>}
      </div>
